...
module FileWatch
  class Tail
    ...
    def _open_file(path, event)
      ...
      @logger.debug("option setting for follow_only_path:  #{@opts[:follow_only_path]}")
      @logger.debug("stat returns #{stat}")
      ...
      if @opts[:follow_only_path]
        ...
        # Because spaces are valid characters in paths and the inode tuples values are
        # delimited by spaces in the .sincedb* files, we need to 'escape' any spaces in the
        # path to avoid derailing the parsing in _sincedb_open.  Since NULL (\0) is NOT a
        # valid path character in LINUX (one of the few), we'll replace spaces with that
        # to create a key that can be parsed safely
        inode = [path.gsub(/ /, "\0\0").gsub(/\n/, "\0\1"), stat.dev_major, stat.dev_minor]
        @logger.debug("follow_only_path: inode constructed as #{inode}")
      else
        if @iswindows
